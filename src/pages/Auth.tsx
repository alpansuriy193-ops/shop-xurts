...
const Auth = () => {
  ...
  const emailSchema = z.string().trim().email(t("authInvalidEmail")).max(255);
  const passwordSchema = z.string().min(6, t("authPasswordMin")).max(72);
  const nameSchema = z.string().trim().min(1, t("authNameRequired")).max(80);
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (!emailRes.success || !passRes.success) {
      toast({ title: t("authInvalidData"), description: emailRes.error?.issues[0]?.message ?? passRes.error?.issues[0]?.message });
      return;
    }
    setLoading(true);
    ...
    setLoading(false);
    if (error) {
      toast({ title: t("authSignInFailed"), description: error.message });
    } else {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (error) {
      toast({ title: t("authSignUpFailed"), description: error.message });
    } else {
      toast({ title: t("authAccountCreated"), description: t("authAccountCreatedDesc") });
      navigate("/");
    }
  };
      ...
      <section className="container-full py-16 md:py-24 min-h-[70vh] flex items-center justify-center">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            <p className="text-[11px] font-semibold tracking-[0.3em] uppercase text-primary mb-3">{t("authAccount")}</p>
            <h1 className="font-serif text-4xl md:text-5xl text-foreground">{t("authWelcome")}</h1>
          ...
          <Tabs defaultValue="signin" className="w-full">
            <TabsList className="grid w-full grid-cols-2 rounded-none h-11">
              <TabsTrigger value="signin" className="text-xs tracking-[0.15em] uppercase rounded-none">{t("authSignIn")}</TabsTrigger>
              <TabsTrigger value="signup" className="text-xs tracking-[0.15em] uppercase rounded-none">{t("authSignUp")}</TabsTrigger>
            </TabsList>

            <TabsContent value="signin" className="mt-8">
              <form onSubmit={handleSignIn} className="space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="si-email" className="text-[11px] tracking-[0.2em] uppercase">{t("authEmail")}</Label>
            ...
            <TabsContent value="signup" className="mt-8">
              <form onSubmit={handleSignUp} className="space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="su-name" className="text-[11px] tracking-[0.2em] uppercase">{t("authName")}</Label>
                  <Input id="su-name" value={signUpName} onChange={(e) => setSignUpName(e.target.value)} className="rounded-none h-11" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="su-email" className="text-[11px] tracking-[0.2em] uppercase">{t("authEmail")}</Label>
          ...
          <p className="text-center text-xs text-muted-foreground mt-6">
            <Link to="/" className="hover:text-foreground transition-colors">{t("authBackToHome")}</Link>
          </p>
        </div>
      </section>
    </Layout>
